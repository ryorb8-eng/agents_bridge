---
description: >-
  Chain brainstorm Q<>A terhadap Claude via bridge: kirim pertanyaan dari
  claude_questions_import/temp_questions_single.md, tunggu balasan, verifikasi,
  ...
  stuck/error/keputusan manusia. Sisi Claude (claude.ai), terpisah dari
  /webchain-gpt (ChatGPT).
---

# /webchain-claude — autonomous brainstorm Q<>A chain (Claude via bridge)
...
Ini adalah sisi **Claude** (`claude.ai`) — mirip `/webchain-gpt` tapi menargetkan
transport + data dir Claude yang terpisah.

Instance yang dipakai (CWD only):
```
GE  = brainstrom/chrome_win11/from_projects/optikmata-web/title_topic/Geometry_Engine
CQI = $GE/claude_questions_import   # temp_questions_all.md, temp_questions_single.md, log_questions_<dd-mm-yy>.md
CAI = $GE/claude_answers_import     # temp_answers.md, temp_knowledges/VERIFY-Q<n>.md, bank_knowledges/
```
...
- Baca `CQI/temp_questions_all.md` → dapat urutan `Q1, Q2, …` (antrian).
- Baca `CQI/log_questions_<dd-mm-yy>.md` → Q mana yang SUDAH dikirim/dijawab.
- `CAI/temp_answers.md` = akumulasi balasan mentah (buat kosong bila belum ada).
...
- baca `web-dom-general` dulu (shared rules), lalu `web-dom-claude` (Claude specifics);
- ambil teks pertanyaan dari `CQI/temp_questions_single.md` (isi = Q saat ini);
- kirim via `claude/bridge-cdp-claude_continue.ts` `BRIDGE_MODE=send BRIDGE_PROMPT="…"`
  (human-like: no Em Dash, ≤50k char, fokus pakai trik `r`+Backspace, delay 0.5s);
- tunggu generasi stabil, lalu baca balasan;
- **APPEND verbatim** balasan ke `CAI/temp_answers.md` dengan header:
  ```
  ## Q<n> — <judul>
  <timestamp>
  <balasan verbatim>
...
- tulis VERDIK → `CAI/temp_knowledges/VERIFY-Q<n>.md` (file EKSKLUSIF per Q);
...
- Append ke `CQI/log_questions_<dd-mm-yy>.md`:
  `- [<waktu>] Q<n> <judul> → <KEEP|REJECT|PARTIAL> (<conf%)`.
- Update `CQI/temp_questions_single.md` → isi Q berikutnya (dari `temp_questions_all.md`).
...
- Baca semua `VERIFY-Q<n>.md`; untuk tiap KEEP/PARTIAL, pindahkan claim ke
  `CAI/bank_knowledges/` (curated, dedupe, normalize).
...
|---|---|---|
| `CQI/temp_questions_single.md` | orchestrator (advance) | semua subagent |
| `CQI/log_questions_<dd-mm-yy>.md` | orchestrator (advance) | semua subagent |
| `CAI/temp_answers.md` | bridge-operator (send) | semua subagent |
| `CAI/temp_knowledges/VERIFY-Q<n>.md` | knowledge-verifier(n) | verifier lain / orchestrator |
| `CAI/bank_knowledges/*` | orchestrator (curate) | semua subagent |
| `claude/bridge-cdp-claude_continue.ts` / CDP session | bridge-operator (1 saat kirim) | — |
...
> CATATAN: transport Claude pakai **fokus trik** (`r` → 0.5s → Backspace → 0.5s) bukan
> `Shift+Esc` seperti ChatGPT — lihat `web-dom-claude` §1.
