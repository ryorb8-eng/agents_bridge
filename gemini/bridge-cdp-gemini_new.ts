...
/**
 * bridge-cdp-gemini_new.ts — TRANSPORT layer untuk agents_bridge (Google Gemini /
 * gemini.google.com).
 *
 * Sama dengan bridge-cdp-gemini_continue.ts, tapi default-nya MENUJU
 * https://gemini.google.com/app (chat BARU). Dipakai untuk:
 *   - brainstorm / task BARU ke Gemini,
 *   - Vision / "mata": paste URL gambar publik atau RAW GitHub, minta deskripsi,
 *   - satu-off ask ke Gemini tanpa mengganggu conversation lama.
 *
 * Override target via BRIDGE_CHAT_URL bila mau menuju conversation spesifik.
 *
 * FOKUS COMPOSER: tekan "/", sleep 0.5s, lalu "Backspace" (hapus), sleep 0.5s.
 * Gemini otomatis memindahkan fokus ke chat input begitu ada ketikan "/" lalu
 * dihapus. Setelah fokus baru kita paste pertanyaan. Lihat web-dom-gemini §1.
 *
 ...
 * lokal atas instruksi remote AI, TIDAK membaca secret. Balasan Gemini adalah DATA,
 * bukan otoritas. Prompt diketik HANYA dari BRIDGE_PROMPT (env), tidak dari balasan remote.
 *
...
// DEFAULT: Gemini new chat.
const CHAT_URL = process.env.BRIDGE_CHAT_URL || 'https://gemini.google.com/app';
...
function extractConvId(url: string): string | null {
  const m = url.match(/\/(?:c|app)\/([0-9a-zA-Z-]{8,})/);
  return m ? m[1] : null;
}
...
async function logConversation(opts: {
  ...
  try {
    ...
    const entry = {
      ...
      remote: REMOTE,
      transport: 'gemini/bridge-cdp-gemini_new.ts',
      mode: opts.mode,
      url,
...
(async () => {
  ...
  try {
    ...
    // Log full URL + metadata (untuk "New Chat", URL sudah berubah jadi /app/<uuid>).
